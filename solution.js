...
const constructMaximumBinaryTree = function (nums) {
  ...
  const l = nums.slice(0, positionMax);
  const ri = nums.slice(positionMax + 1, nums.length);

  const r = new TreeNode(currMax);
  ...
  function dfs(root, left, right) {
    if (left.length >= 1) {
      ...
      dfs(
        root.left,
        left.slice(0, positionMax),
        left.slice(positionMax + 1, left.length),
      );
    }
    ...
    if (right.length >= 1) {
      ...
      dfs(
        root.right,
        right.slice(0, positionMax),
        right.slice(positionMax + 1, right.length),
      );
    }
  }
